/*
 * @Author: gaoyong06@qq.com
 * @Date: 2020-11-05 17:30:23 
 * @Last Modified by: gaoyong06@qq.com
 * @Last Modified time: 2020-11-05 17:30:23
 */

import 'package:flutter/material.dart';

class AppColors {
  AppColors._();

  // 主色+辅色
  // #5791F3 用于界面主色：按钮、icon、文字链接、重要提示等
  static const Color cl5791F3 = Color(0xFF5791F3);

  // #00CB91 用于界面辅色、icon等
  static const Color cl00CB91 = Color(0xFF00CB91);

  // #FF9C28 少量用于界面：高亮注释标签等
  static const Color clFF9C28 = Color(0xFFFF9C28);

  // 主色+辅色
  // #333333 标题、重要信息、列表、标签文字、输入框文字等
  static const Color cl333333 = Color(0xFF333333);

  // #666666 注释文字
  static const Color cl666666 = Color(0xFF666666);

  // #999999 注释、时间、次要文字
  static const Color cl999999 = Color(0xFF999999);

  // #D8D8D8 表单未输入时提示文字
  static const Color clD8D8D8 = Color(0xFFD8D8D8);

  // 背景/分割线
  // #FFFFFF 背景色纯白

  //纯黑色
  static const Color cl000000 = Color(0xFF000000);

  //灰黑色阴影
  static const Color cl27000000 = Color(0x27000000);

  static const Color cl1A000000 = Color(0x1A000000);

  static const Color clF0F0F0 = Color(0xFFF0F0F0);

  static const Color clF4F5F7 = Color(0xFFF4F5F7);
  //删除文字颜色
  static const Color clFF0000 = Color(0xFFFF0000);
  //90%黑色透明
  static const Color cl19000000 = Color(0x19000000);

  static const Color cl07C160 = Color(0xFF07C160);
  static const Color cl1989FA = Color(0xFF1989FA);
  static const Color clFF976A = Color(0xFFFF976A);
  static const Color clEE0A24 = Color(0xFFEE0A24);

  static const Color clFFFFFF = Color(0xFFFFFFFF);
  static const Color clEDEDF0 = Color(0xFFEDEDF0);
  static const Color clC8C9CC = Color(0xFFC8C9CC);
  static const Color clDCDEE0 = Color(0xFFDCDEE0);
  static const Color clF7F8FA = Color(0xFFF7F8FA);

  static const Color cl646566 = Color(0xFF646566);
  static const Color cl969799 = Color(0xFF969799);
  static const Color clEBEDF0 = Color(0xFFEBEDF0);

  static const Color cl455A64 = Color(0xFF455A64);
  static const Color cl66455A64 = Color(0x66455A64);
  static const Color cl323233 = Color(0xFF323233);
  static const Color cl7D80AEFF = Color(0x7D80AEFF);
}
